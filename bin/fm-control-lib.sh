...
# kiro are crewmate/scout adapters only: none has a primary supervision protocol,
...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|gemini|rovo|agy|kiro) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|omp|kimi|cursor|gemini|muse|rovo|agy|kiro) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|kiro) ;;
    *) return 1 ;;
  esac
}
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    ...
    codex|pi|pi-signed|omp|gemini|agy|kiro) printf '/quit' ;;
    *) return 1 ;;
  esac
}
