...
The derived grace below accounts only for the poll, so it does not bound a check: on a home whose `FM_CHECK_TIMEOUT` exceeds that grace, a healthy watcher inside a single check still reads stale and the arm still refuses to attach.
That gap is not closed here: a home that raises `FM_CHECK_TIMEOUT` above its derived grace can still have a healthy watcher read stale while it waits out one registered check, and closing it would mean deriving the grace from the check timeout as well as the poll.
