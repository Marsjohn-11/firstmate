...
- Busy footer: `› Kiro is working · Type to steer · Ctrl+S to queue`. The delivery guard matches the ANCHORED form of that footer, the harness-named phrase plus its `· Type to steer` separator, not the bare phrase and not the bare `esc to cancel` token kiro also renders in its tool region and shares with agy. It is never a recorded worker state, and its one consumer is the harness-less union in `FM_DELIVERY_BUSY_REGEX_DEFAULT` that the tmux submit core reads to acknowledge a submit.
The separator is required because the bare phrase appears verbatim in this record and in the kiro reference page, so a worker that greps or quotes either one puts an acknowledgement token on its own pane; anchoring narrows that self-inflicted surface without closing it, since both files also spell the full footer.
A pane narrow enough to truncate the separator loses the acknowledgement and defers the steer instead, which is the deliberately chosen direction: a deferral is visible and retried, while a false positive records an undelivered steer as delivered and loses it silently.
