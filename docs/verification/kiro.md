...
- Whether the union's anchored kiro literal matches a live kiro pane. The anchor is derived from the footer template in the `kiro-cli-chat` binary on kiro-cli 2.22.2-nightly.2 on macOS, so it rests on vendor source rather than on a rendered pane, and no live run has yet read a real footer through the union on the Linux desk. The portable regression cannot settle it either, because its fixtures are built from the same separator byte `bin/fm-composer-lib.sh` spells, so a separator that does not match vendor output passes CI green. If the literal is wrong the union never matches a live kiro pane, the submit core's post-Enter read stays idle, and every steer to a kiro worker defers instead of landing - visible and retried rather than silently lost, which is why the anchor requires only what all three template variants share.
