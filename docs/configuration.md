...
FM_COMPOSER_GHOST_GRAY_LUMA_MAX=180   # fleet-wide: the higher luminance ceiling a NEAR-ACHROMATIC truecolor foreground gets instead of FM_COMPOSER_GHOST_LUMA_MAX, because gray is where harnesses draw placeholders (kiro at 158, rovo at 163) while chromatic real glyphs stay out of reach. It only ever RAISES the applicable ceiling - a near-gray run takes the higher of the two, so raising FM_COMPOSER_GHOST_LUMA_MAX past this still widens the strip on gray text
FM_COMPOSER_GHOST_GRAY_SPREAD_MAX=12   # fleet-wide: max channel spread (max channel minus min channel, 0-255) for a TRUECOLOR foreground to count as near-achromatic and take the gray ceiling above; anything more saturated keeps FM_COMPOSER_GHOST_LUMA_MAX
