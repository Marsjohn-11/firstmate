...
A kiro hook command must be one unquoted token, so a kiro spawn refuses before launch when that per-task home path carries any character a shell would act on unquoted and asks for a `FM_HOME` or `FM_STATE_OVERRIDE` of only letters, digits and `. _ - / + = : @ , %`.
