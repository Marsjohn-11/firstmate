...
FM_COMPOSER_GHOST_GRAY_LUMA_MAX=180   # fleet-wide: the higher luminance ceiling a NEAR-ACHROMATIC foreground gets instead of FM_COMPOSER_GHOST_LUMA_MAX, because gray is where harnesses draw placeholders (kiro at 158, rovo at 163) while chromatic real glyphs stay out of reach. Applies to a near-achromatic truecolor run and to a 256-colour index that is gray in the standard xterm palette (the 232-255 ramp and the 6x6x6 cube diagonal), which is how the same harness draws the same gray on a pane with no truecolor. It only ever RAISES the applicable ceiling - a near-gray run takes the higher of the two, so raising FM_COMPOSER_GHOST_LUMA_MAX past this still widens the strip on gray text
FM_COMPOSER_GHOST_GRAY_SPREAD_MAX=12   # fleet-wide: max channel spread (max channel minus min channel, 0-255) for a TRUECOLOR foreground to count as near-achromatic and take the gray ceiling above; anything more saturated keeps FM_COMPOSER_GHOST_LUMA_MAX. A gray palette index has zero spread by definition, so this knob does not gate it
