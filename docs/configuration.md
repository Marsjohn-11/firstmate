...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true`, pins `commands.lint` to `bin/fm-lint.sh`, and pins `commands.test` to `bin/fm-no-mistakes-test.sh`.
...
The [`firstmate-coding-guidelines` skill](../.agents/skills/firstmate-coding-guidelines/SKILL.md#no-mistakes-test-configuration) owns the validation policy.
The test command derives the same portable and real-Herdr lane inventory used by CI, proves every discovered `tests/*.test.sh` file is assigned exactly once, runs those lanes in isolated concurrent checkouts, and reports aggregate passed, failed, and skipped counts.
