...
Seeding the per-task home from the operator's global config so those MCP servers and skills carry over is follow-up work, not a property of the harness: `--agent` being name-only forces config out of the worktree, but it does not require the home to start empty.
