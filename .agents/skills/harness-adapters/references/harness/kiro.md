...
| Composer | Bare `›` (U+203A) agent-glyph row, the same glyph codex draws. The `ask a question or describe a task` placeholder is truecolor near-gray at luminance 158, which the shared ghost strip removes under its near-achromatic ceiling, so a real idle pane reads `empty` (verified against a live capture with tmux's own descriptor). Before that ceiling existed the placeholder survived the strip and the pane read `pending`, which suppressed every steer; `../../../../../docs/verification/kiro.md` owns that history. kiro registers no entry in the fleet-wide idle-placeholder set, which changes no verdict here. |
