...
| Rendered tail | Busy composer footer is `Kiro is working · Type to steer · Ctrl+S to queue`; idle shows `Trust All Tools active ... /quit to exit` or the `ask a question or describe a task` placeholder. The ANCHORED form of that footer is matched - the harness-named phrase plus its `· Type to steer` separator - never the bare phrase and never the bare `esc to cancel` token kiro shares with agy, because the bare phrase appears in this page and in the verification record and a worker quoting either would otherwise acknowledge a submit that never landed. A pane narrow enough to truncate the separator loses the acknowledgement and defers the steer, which is the safe direction. It is a DELIVERY guard in `../../../../../bin/fm-composer-lib.sh`, never a worker state source, and its one consumer is the harness-less union in `FM_DELIVERY_BUSY_REGEX_DEFAULT` that the tmux submit core reads to acknowledge a submit; kiro declares no per-harness signature, because the harness-scoped readers are away-mode injection (primary) and pending-reply observation (secondmate) and kiro can be neither. |
