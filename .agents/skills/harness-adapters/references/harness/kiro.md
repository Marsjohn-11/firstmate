...
| Composer | Bare `›` (U+203A) agent-glyph row, the same glyph codex draws, which the shared classifier reads as `empty` on a synthetic styled reconstruction. The dim `ask a question or describe a task` placeholder is ghost text a styled capture strips back to that glyph, and an unstyled capture degrades it to `unknown` through the bare-row rule; kiro registers no entry in the fleet-wide idle-placeholder set, which only changes a verdict on a bordered or left-bar composer. A live run instead measured `pending` on the real pane, so steering a kiro worker is an OPEN DEFECT and the `empty` verdict is not established against the real tool - see `../../../../../docs/verification/kiro.md` before relying on this row. |
