...
On a name collision the WORKSPACE copy wins, so a target repo shipping `.kiro/agents/firstmate.json` shadows the firstmate-owned per-task config and leaves a worker with no hooks and a busy record nothing closes; kiro prints a conflict warning on the pane and `../../../../../docs/verification/kiro.md` owns the measurement.
...
A canonical kiro spawn therefore refuses before launch when the resolved `state/<id>.kiro-home` path carries any character a shell would act on unquoted, naming that path and asking for a `FM_HOME` or `FM_STATE_OVERRIDE` of only letters, digits and `. _ - / + = : @ , %`.
Whitespace breaks the token under either interpretation and the rest break it under the shell reading, but all of them reach one outcome: without the hooks kiro has no other state source, so a pane that could never clear its busy record is never started.
The rule is an allowlist rather than a denylist of known-bad characters, so a path shape nobody has reasoned about is refused instead of launched.
